...
public:
	...
	BIO *bio_err;				// OpenSSL BIO错误处理
	static char * pass;		// OpenSSL密码回调函数使用
	SSL_CTX *ctx;		// OpenSSL上下文
...
public:
	void HandleEndianness(PREQUEST pReq);							//判断字节序
	void HandleGet(PREQUEST pReq);
	void HandleHead(PREQUEST pReq);
	void HandlePost(PREQUEST pReq);
